...
var blimp = {
  ...
  answer : {
    ...
    is_match : function(input){
      ...
      if(!isMatch){ //Wrong answer
        ...
        blimp.data.used_s.push(blimp.country.current.cca3);
        blimp.save();
  ...
  country : {
    ...
    get : function(id){
      ...
      if(!id){
        ...
        var region = (blimp.active_region == null ? c.region : blimp.active_region);
        while(blimp.data.used_s.indexOf(c.cca3) > -1 || region !== c.region) {
          region = (blimp.active_region == null ? c.region : blimp.active_region);
          c = blimp.data.random(blimp.data.countries);          
        }
      } else {
  ...
  data : {
    ...
    set_used : function(countries){
      $.each(countries,function(i,n){
        console.log(n);
        c = blimp.country.get(n);
        blimp.data.used.push(c);
        blimp.data.used_s.push(c.cca3);
        var l = L.geoJson(blimp.country.current.geo,{
          style: map_style.highlight,
          //onEachFeature: onEachFeature,
        });
        map.addLayer(blimp.country.layer);
  ...
  load_saved :{
    ...
    load : function(){
      if (simpleStorage.canUse()) {
        data = simpleStorage.get("blimp");
        blimp.scoring.load(data.score);
        blimp.data.used = data.used;
        $.each(blimp.data.used,function(i,n){
          var layer = L.geoJson(n.geo,{
            style: map_style.success,
          });
          map.addLayer(layer);
        });
  ...
  scoring : {
    ...
    load : function(scores){
      this.total = scores[0];
      this.questions = scores[1];
      this.max = 10*this.questions;
      
      $('.points-ratio').text(this.total);
  ...
  start : function(){
    if(blimp.data.used.length < (blimp.data.countries.length - 5)){
      this.country.get();
      $("#answer-input").focus();
    } else {
...
var filter = {
  region : function(){
    var ret = [];
    $.each(blimp.data.countries, function(i,n){
      if(ret.indexOf(n.region) == -1) ret.push(n.region);
    });
    console.log(ret);
  },
  sub_region : function(){
    var ret = [];
    $.each(blimp.data.countries, function(i,n){
      if(ret.indexOf(n.subregion) == -1) ret.push(n.subregion);
    });
    console.log(ret);
  }
}
